...
def buzz(frequency, length,buzzer_pin):
...
def play(buzzer_pin,melody,tempo,pause,pace=0.800):
    for i in range(0, len(melody)):     # Play song
        noteDuration = pace/tempo[i]
        buzz(melody[i],noteDuration,buzzer_pin)    # Change the frequency along the song note
        pauseBetweenNotes = noteDuration * pause
        sleep(pauseBetweenNotes)

def play_music(buzzer_pin):
    play(buzzer_pin,underworld_melody, underworld_tempo, 1.3, 0.800)
