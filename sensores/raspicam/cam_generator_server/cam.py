...
class Camara():
    def __init__(self):
        ...
        print('Camara creada de manera exitosa')

    def get_frame(self):
        import pudb ; pudb.set_trace()
        rawCapture = PiRGBArray(self.cam, size=(640, 480))
        for cuadro in self.cam.capture_continuous(rawCapture, format="bgr", 
            use_video_port=True):
            cuadro = cuadro.array
            ...
            if len(coordenadas_rostros):
                ...
                for i, rostro in enumerate(rostros): 
                    ...
                    print("Prediccion: " \
                    + self.dicc_etiquetas[prediccion].capitalize() \
...
def recolectarConjunto():
    ...
    etiquetas = []
    dicc_etiquetas = {}
    gente = [persona for persona in os.listdir('/home/pi/tesis/'
        'Codigos de dispostivos/RPi3Peter/raspicam/personas/')]
    for i,persona in enumerate(gente):
        dicc_etiquetas[i] = persona
        for imagen in os.listdir('/home/pi/tesis/Codigos de dispostivos/'
            'RPi3Peter/raspicam/personas/' + persona):
            imagenes.append(cv2.imread('/home/pi/tesis/Codigos de dispostivos/'
                'RPi3Peter/raspicam/personas/' + persona +'/' +imagen,0))
            etiquetas.append(i)
